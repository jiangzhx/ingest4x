...
| sink commit 失败 | 不进入隔离；该 sink checkpoint 不前进，后续重试 |

`blackhole` 作为生产级诊断 sink：
`mode = "ok"` 丢弃后直接前进 checkpoint；`mode = "slow"` 在 `delay_ms` 后成功；`mode = "fail"` 在 commit 前返回 sink 失败，用于验证失败 sink 下 checkpoint 不前进并形成 WAL backlog。
