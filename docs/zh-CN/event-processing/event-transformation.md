...
转换脚本在 WAL 重放阶段执行：它会通过 `event` 上的校验 helper 完成字段校验、可选地变更/扩展事件，并决定事件送达哪些 sink。
...
`event` 上可用的校验 helper 见 [校验 helper](event-validation.md)。
...
客户传入的 header（例如 `authorization`）会进入 processor request context。ingest4x 只会在写 WAL 前移除自身使用的 `x-ingest-token` header。
...
| 校验 helper 失败 | 抛出普通 Rhai 运行时错误；脚本可自行 `try/catch` 并发到错误 sink |
| 处理脚本运行失败 | 重放将记录为处理失败并进入隔离 |
| `emit` 目标缺失 | 重放将按投递计划错误进入隔离 |
| sink commit 失败 | 不进入隔离；pipeline checkpoint 不前进，后续重试整个 replay window |

`blackhole` 作为生产级诊断 sink：
`mode = "ok"` 丢弃后允许 checkpoint 前进；`mode = "slow"` 在 `delay_ms` 后成功；`mode = "fail"` 在 commit 前返回 sink 失败，用于验证失败 sink 会阻塞 pipeline checkpoint 并形成 WAL backlog。

## 默认脚本

默认 seed 的基础处理脚本会在 `process(...)` 内联校验并决定投递：
...
简而言之：`process(...)` 单脚本同时负责校验、转换和投递计划；校验 helper 只负责字段检查与比较。
