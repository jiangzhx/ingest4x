...
`flush_max_interval` 与 `flush_max_records` 控制 group commit。多个请求可以在一个短窗口内合并 flush，降低 sync 频率。`no_sync = false` 时，每个请求会等待自己的 flush 完成后才返回成功。

当 `wal.no_sync = true` 时，append 在写入内存缓冲后返回，flush 由后台异步执行。该模式可降低延迟，但进程在 flush 前崩溃会丢失最近一批未刷盘数据；因此它不是强持久 ACK。
...
- `checkpoints/*.json`：每个 event sink 的重放 checkpoint。
...
当下一个 frame 会超过 `wal_segment_max_bytes` 时，writer 会创建新 segment。默认：
...
服务启动时会启动 WAL 重放循环。每次运行默认读取一批记录（默认 batch size `1024`）。

重放仍然逐条 WAL record 执行 rules 与 processor，因为 Rhai 当前每次接收一个 JSON event。processor 输出校验完成后，系统会在当前 replay window 内按 sink 暂存 delivery，每个 sink 最终收到一次 `send_batch` 调用。
...
3. 只有该 batch 达到 sink 自身定义的 commit 点后，才推进该 sink checkpoint。

每种 sink 自己定义 commit 完成标准。Kafka 可以把 broker delivery report 成功视为 commit；本地文件 sink 必须等最终文件提交完成；对象存储 sink 必须等 upload 完成或 multipart complete 成功。WAL replay 只观察 sink runtime 的结果：`Ok` 允许 checkpoint 前进，`Err` 保持 checkpoint 滞后并等待重试。
...
新 sink 尚无 checkpoint 时，`auto_offset_reset` 决定起点：

| 值 | 行为 |
| --- | --- |
| `earliest` | 从可读 WAL 最早偏移重放 |
| `latest` | 初始化到当前 WAL 尾部，只消费新事件 |

默认 seed 对 `events`、`events_error`、`loadtest_events` 使用 `latest`。

若已有 checkpoint 落后于当前 WAL floor，可能是旧 segment 已清理，系统会按该 sink 的 `auto_offset_reset` 重置。

## 保留与清理

只有当所有已启用 sink 的最小 checkpoint 已经过某个 segment 后，该 segment 才可清理。实践含义：

- 快速 sink checkpoint 持续推进。
- 慢速/失败 sink 的 checkpoint 拖慢老 segment 的回收。
- 全局清理水位线为所有活跃 sink 的最小 checkpoint。

长时间失败的 sink 会阻塞 WAL 空间复用，处理方式是修复/停用 sink，或显式重置 checkpoint。
...
sink commit 失败不进入隔离。若 sink 在达到自身 commit 点前失败，其 checkpoint 不前进，重放会按退避重试；其它 sink 若已成功 commit 同条记录则可继续推进各自 checkpoint。
...
| `wal_checkpoint_lsn` | 当前 sink 最小 checkpoint |
...
| --- | --- |
| `wal.dir` | WAL 数据目录 |
| `wal.flush_max_interval` | 最长缓冲 flush 间隔 |
| `wal.flush_max_records` | 强制 flush 的最大记录数 |
| `wal.no_sync` | 写入时是否跳过同步等待 |
| `wal.wal_segment_max_bytes` | 分段最大大小 |
| `wal.min_free_bytes` | 最小空闲阈值，非零时生效 |
| `wal.checkpoint.flush_interval` | 两次 checkpoint flush 最大间隔 |
| `wal.checkpoint.flush_records` | checkpoint flush 的最大记录数 |
| `wal.checkpoint.flush_bytes` | checkpoint flush 的最大字节数 |
...
- WAL 不会生成业务事件 ID。
- WAL 不会去重重复提交。
- 重放可能在 sink commit 与 checkpoint 写入之间重复重试同一记录；下游需要 exactly-once 时应使用事件 ID 或业务主键。
