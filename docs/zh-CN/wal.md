...
sink 投递失败不进入隔离。sink 失败时，其 checkpoint 不前进，重放会按退避重试；其它 sink 若已成功处理同条记录则可继续推进各自 checkpoint。
...
- WAL 不会生成业务事件 ID。
- WAL 不会去重重复提交。
- 重放可能在 sink 投递与 checkpoint 写入之间重复重试同一记录；下游需要 exactly-once 时应使用事件 ID 或业务主键。
